...
import com.sixray.cepat.ui.MainActivity
...
object NotificationManager {
    ...
    fun startSpeedNotification(currentConfig: ProfileItem?) {
        ...
        if (speedNotificationJob != null || SixRayServiceManager.isRunning() == false) return
        ...
        speedNotificationJob = CoroutineScope(Dispatchers.IO).launch {
            while (isActive) {
                ...
                outboundTags?.forEach {
                    val up = SixRayServiceManager.queryStats(it, AppConfig.UPLINK)
                    val down = SixRayServiceManager.queryStats(it, AppConfig.DOWNLINK)
                    if (up + down > 0) {
                        appendSpeedString(text, it, up / sinceLastQueryInSeconds, down / sinceLastQueryInSeconds)
                        proxyTotal += up + down
                    }
                }
                val directUplink = SixRayServiceManager.queryStats(AppConfig.TAG_DIRECT, AppConfig.UPLINK)
                val directDownlink = SixRayServiceManager.queryStats(AppConfig.TAG_DIRECT, AppConfig.DOWNLINK)
    ...
    private fun getService(): Service? {
        return SixRayServiceManager.serviceControl?.get()?.getService()
    }
}