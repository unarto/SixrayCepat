...
import com.sixray.cepat.dto.RulesetItem
import com.sixray.cepat.dto.SixRayConfig
import com.sixray.cepat.dto.SixRayConfig.OutboundBean
import com.sixray.cepat.dto.SixRayConfig.OutboundBean.OutSettingsBean
import com.sixray.cepat.dto.SixRayConfig.OutboundBean.StreamSettingsBean
import com.sixray.cepat.dto.SixRayConfig.RoutingBean.RulesBean
import com.sixray.cepat.extension.isNotNullEmpty
import com.sixray.cepat.fmt.CustomFmt
import com.sixray.cepat.fmt.HttpFmt
...
object SixRayConfigManager {
     ...
     */
    fun getSixRayConfig(context: Context, guid: String): ConfigResult {
        try {
     ...
     * @return A SixRayConfig object containing the combined configuration of all specified servers,
     *         or null if the operation fails (e.g., no valid configurations found, parsing errors)
     */
    fun genSixRayConfig(context: Context, guidList: List<String>): SixRayConfig? {
        try {
     ...
     * @return A ConfigResult object containing the configuration details or indicating failure.
     */
    fun getSixRayConfig4Speedtest(context: Context, guid: String): ConfigResult {
        try {
            val config = MmkvManager.decodeServerConfig(guid) ?: return ConfigResult(false)
            return if (config.configType == EConfigType.CUSTOM) {
                getV2rayCustomConfig(guid, config)
    ...
    private fun getV2rayNormalConfig(context: Context, guid: String, config: ProfileItem): ConfigResult {
        ...
        val v2rayConfig = initSixRayConfig(context) ?: return result
    ...
    private fun genV2rayMultipleConfig(context: Context, configList: List<ProfileItem>): SixRayConfig? {
        ...
        val v2rayConfig = initSixRayConfig(context) ?: return null
        v2rayConfig.log.loglevel = MmkvManager.decodeSettingsString(AppConfig.PREF_LOGLEVEL) ?: "warning"
        ...
        val outboundsList = mutableListOf<SixRayConfig.OutboundBean>()
        var index = 0
     ...
     * @return SixRayConfig object parsed from the JSON configuration, or null if the configuration is empty
     */
    private fun initSixRayConfig(context: Context): SixRayConfig? {
        val assets = initConfigCache ?: Utils.readTextFromAssets(context, "v2ray_config.json")
        if (TextUtils.isEmpty(assets)) {
            return null
        }
        initConfigCache = assets
        val config = JsonUtil.fromJson(assets, SixRayConfig::class.java)
        return config
    }
     ...
     */
    private fun getInbounds(v2rayConfig: SixRayConfig): Boolean {
        try {
     ...
     * @param v2rayConfig The V2ray configuration object to be modified
     */
    private fun getFakeDns(v2rayConfig: SixRayConfig) {
        if (MmkvManager.decodeSettingsBool(AppConfig.PREF_LOCAL_DNS_ENABLED) == true
            && MmkvManager.decodeSettingsBool(AppConfig.PREF_FAKE_DNS_ENABLED) == true
        ) {
            v2rayConfig.fakedns = listOf(SixRayConfig.FakednsBean())
        }
    }
     ...
     */
    private fun getRouting(v2rayConfig: SixRayConfig): Boolean {
        try {
     ...
     * @param v2rayConfig The V2ray configuration object to be modified
     */
    private fun getRoutingUserRule(item: RulesetItem?, v2rayConfig: SixRayConfig) {
        try {
     ...
     */
    private fun getCustomLocalDns(v2rayConfig: SixRayConfig): Boolean {
        try {
            if (MmkvManager.decodeSettingsBool(AppConfig.PREF_FAKE_DNS_ENABLED) == true) {
                ...
                v2rayConfig.dns?.servers?.add(
                    0,
                    SixRayConfig.DnsBean.ServersBean(
                        address = "fakedns",
            ...
            if (MmkvManager.decodeSettingsBool(AppConfig.PREF_USE_HEV_TUNNEL ,true) == false) {
                ...
                if (v2rayConfig.inbounds.none { e -> e.protocol == "dokodemo-door" && e.tag == "dns-in" }) {
                    val dnsInboundSettings = SixRayConfig.InboundBean.InSettingsBean(
     ...
     */
    private fun getDns(v2rayConfig: SixRayConfig): Boolean {
        try {
            ...
            // DNS dns
            v2rayConfig.dns = SixRayConfig.DnsBean(
                servers = servers,
                hosts = hosts,
     ...
     */
    private fun getOutbounds(v2rayConfig: SixRayConfig, config: ProfileItem): Boolean? {
     ...
     */
    private fun getPlusOutbounds(v2rayConfig: SixRayConfig, config: ProfileItem): Int? {
        try {
     ...
     */
    private fun getMoreOutbounds(v2rayConfig: SixRayConfig, subscriptionId: String): Boolean {
        //fragment proxy
     ...
     */
    private fun updateOutboundWithGlobalSettings(outbound: SixRayConfig.OutboundBean): Boolean {
        try {
     ...
     */
    private fun getBalance(v2rayConfig: SixRayConfig)
    {
        try {
            ...
            if (MmkvManager.decodeSettingsString(AppConfig.PREF_INTELLIGENT_SELECTION_METHOD, "0") == "0") {
                val balancer = SixRayConfig.RoutingBean.BalancerBean(
                    tag = "proxy-round",
                    selector = listOf("proxy-"),
                    strategy = SixRayConfig.RoutingBean.StrategyObject(
                        type = "leastPing"
                    )
                )
                v2rayConfig.routing.balancers = listOf(balancer)
                v2rayConfig.observatory = SixRayConfig.ObservatoryObject(
                ...
                v2rayConfig.routing.balancers = listOf(balancer)
                v2rayConfig.burstObservatory = SixRayConfig.BurstObservatoryObject(
                    subjectSelector = listOf("proxy-"),
                    pingConfig = SixRayConfig.BurstObservatoryObject.PingConfigObject(
     ...
     */
    private fun updateOutboundFragment(v2rayConfig: SixRayConfig): Boolean {
        try {
     ...
     * @param v2rayConfig The V2ray configuration object to be modified
     */
    private fun resolveOutboundDomainsToHosts(v2rayConfig: SixRayConfig) {
     ...
     */
    private fun convertProfile2Outbound(profileItem: ProfileItem): SixRayConfig.OutboundBean? {
