...
import com.sixray.cepat.service.ServiceControl
import com.sixray.cepat.service.SixRayProxyOnlyService
import com.sixray.cepat.service.SixRayVpnService
...
object SixRayServiceManager {
     ...
     */
    fun isRunning() = coreController.getIsRunning()
...
//        val result = SixRayConfigUtil.getSixRayConfig(context, guid)
//        if (!result.status) return
        ...
        val intent = if ((MmkvManager.decodeSettingsString(AppConfig.PREF_MODE) ?: AppConfig.VPN) == AppConfig.VPN) {
            Intent(context.applicationContext, SixRayVpnService::class.java)
        } else {
            Intent(context.applicationContext, SixRayProxyOnlyService::class.java)
        }
    ...
    fun startCoreLoop(): Boolean {
        ...
        val result = SixRayConfigManager.getSixRayConfig(service, guid)
        if (!result.status)
            return false
        ...
        if (coreController.getIsRunning() == false) {
